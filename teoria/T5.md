...
1. [Patrón MVC para APIs](#1-patrón-mvc-para-apis)
...
5. [Implementación del MVC](#5-implementación-del-mvc)
...
10. [Validación de ObjectId](#10-validación-de-objectid)

---

## 1. Patrón MVC para APIs

### 1.1 ¿Qué es MVC?

El patrón **Model-View-Controller (MVC)** es una arquitectura que separa la aplicación en tres componentes con responsabilidades bien definidas:
...
┌───────────────────────┐       ┌─────────────────────────────┐
│        MODEL          │       │           VIEW              │
│  (Datos y esquemas)   │       │    (Respuesta JSON/HTML)    │
└───────────┬───────────┘       └─────────────────────────────┘
            │
            ▼
┌───────────────────────┐
...
### 1.2 Adaptación para APIs REST

En APIs REST, el MVC se adapta así:

| Componente | Responsabilidad | Ejemplo |
|------------|-----------------|---------|
| **Model** | Estructura de datos, validación, interacción con BD | Schemas de Mongoose |
| **View** | Respuesta al cliente | JSON (`res.json()`) |
| **Controller** | Lógica de negocio, coordinación | Funciones que procesan requests |
| **Routes** | Mapeo URL → Controller | `router.get('/users', getUsers)` |

### 1.3 Ventajas del MVC

- **Separación de responsabilidades**: Cada componente tiene una función clara
- **Mantenibilidad**: Fácil de modificar partes específicas
- **Testabilidad**: Componentes probables de forma aislada
- **Escalabilidad**: Facilita el crecimiento del proyecto
- **Reutilización**: Código reutilizable entre endpoints
...
│   │   └── storage.routes.js
│   ├── schemas/
│   │   ├── user.schema.js        # Validación con Zod
│   │   └── track.schema.js
│   └── utils/
│       ├── handleError.js
│       └── handleStorage.js
...
MongoDB es una base de datos **NoSQL** orientada a documentos. Almacena datos en documentos JSON flexibles (técnicamente BSON), no en tablas rígidas.
...
3. **Crear cluster**:
   ...
   - Elegir proveedor (AWS, GCP, Azure) y región cercana
...
> 💡 Ya no necesitas el paquete `dotenv`. Node.js 20.6+ lo soporta nativamente.

---

## 4. Mongoose 8.x

Mongoose es un ODM (Object Document Mapper) que proporciona una capa de abstracción elegante sobre MongoDB.
...
> 📂 **Código de ejemplo**: [`codigo/T5/src/config/db.js`](../codigo/T5/src/config/db.js)
...
> 📂 **Código de ejemplo**: [`codigo/T5/src/models/user.model.js`](../codigo/T5/src/models/user.model.js) y [`codigo/T5/src/models/track.model.js`](../codigo/T5/src/models/track.model.js)
...
## 5. Implementación del MVC

> 📂 **Código de ejemplo**: [`codigo/T5/src/app.js`](../codigo/T5/src/app.js), [`codigo/T5/src/routes/`](../codigo/T5/src/routes/) y [`codigo/T5/src/controllers/`](../codigo/T5/src/controllers/)
...
**Contras**: Datos duplicados, límite 16MB por documento.
...
### 7.4 ¿Cuándo usar cada estrategia?

| Embedding | Referencias |
|-----------|-------------|
| Datos pequeños y estáticos | Datos grandes o dinámicos |
| Relación 1:1 o 1:pocos | Relación 1:muchos o N:M |
...
> 📂 **Código de ejemplo**: [`codigo/T5/src/utils/handleStorage.js`](../codigo/T5/src/utils/handleStorage.js), [`codigo/T5/src/models/storage.model.js`](../codigo/T5/src/models/storage.model.js) y [`codigo/T5/src/controllers/storage.controller.js`](../codigo/T5/src/controllers/storage.controller.js)

Multer es el middleware estándar para manejar `multipart/form-data` (subida de archivos).
...
> 📂 **Código de ejemplo**: [`codigo/T5/src/middleware/error.middleware.js`](../codigo/T5/src/middleware/error.middleware.js) y [`codigo/T5/src/utils/handleError.js`](../codigo/T5/src/utils/handleError.js)
...
## 10. Validación de ObjectId

> 📂 **Código de ejemplo**: [`codigo/T5/src/middleware/validate.middleware.js`](../codigo/T5/src/middleware/validate.middleware.js)
...
- `Book`: title, isbn (único), author (ref), genre (enum), publishedYear, pages, available
...
Extiende el storage para crear álbumes:
...
- `GET /api/albums/public` - Álbumes públicos
...
Un millonario excéntrico quiere recrear la experiencia de los videoclubs de los 90s con una API REST moderna.

**Ubicación:** `ejercicios/T5/`

**Requisitos:**
1. Modelo Movie con validaciones Mongoose
2. CRUD completo + operaciones de alquilar/devolver
3. Estadísticas de películas más alquiladas

**Endpoints:**
| Método | Ruta | Descripción |
|--------|------|-------------|
| GET | /api/movies | Listar (filtro: ?genre=comedy) |
| GET | /api/movies/:id | Obtener una |
| POST | /api/movies | Crear película |
| POST | /api/movies/:id/rent | Alquilar |
| POST | /api/movies/:id/return | Devolver |
| GET | /api/movies/stats/top | Top 5 alquiladas |

**Conceptos a aplicar:**
- Express + MongoDB + Mongoose
- Modelos con validaciones
- Operaciones de negocio (alquilar/devolver)
...
| Schema | Define estructura y validación de documentos |
...
En el **Tema 6 - Validación Avanzada y Manejo de Errores** veremos:
- Validación avanzada con Zod (refine, transform)
